...
experiment_id = 'clev001'
file_name = "experiments/configs/cleveland.yaml"

with open(file_name, "r") as yamlfile:
    data = yaml.load(yamlfile, Loader=yaml.FullLoader)
    logger.info(f"Read successful file {file_name}")
    targ = data[experiment_id]['target']
    dataset = Dataset('cleveland', data[experiment_id]['features'], data[experiment_id]['target'], None)
    dataset_nosex = Dataset('cleveland', data[experiment_id]['features'], data[experiment_id]['target'], "nosex")
    ...
    plt.savefig("results/plots/clev_shap.png", bbox_inches='tight', pad_inches=0)

    shap.dependence_plot("sex", shap_values[1], dataset.X_train, xmin=xmin, xmax=xmax, show=False)
    plt.savefig("results/plots/clev_shap_sex_depend.png", bbox_inches='tight', dpi=600)

    shap.group_difference_plot(shap_values[1], sex, dataset.X_train.columns, xmin=xmin, xmax=xmax, show=False)
    plt.savefig("results/plots/clev_shap_sex_group_diff.png", bbox_inches='tight', dpi=600)

    shap.group_difference_plot(shap_values_nosex[1], sex, dataset_nosex.X_train.columns, xmin=xmin, xmax=xmax, show=False)
    plt.savefig("results/plots/clev_shap_sex_group_diff_nosex_features.png", bbox_inches='tight', dpi=600)

    shap.group_difference_plot(shap_values_nosex[1].sum(1), sex, xmin=xmin, xmax=xmax, show=False)
    plt.savefig("results/plots/clev_shap_sex_group_diff_nosex.png", bbox_inches='tight', dpi=600)

    # male
    logger.info("Prediction male ---------")
    dataset = Dataset('cleveland', data[experiment_id]['features'], data[experiment_id]['target'], gender_sep='male')
    clf = RandomForestClassifier(max_depth=data[experiment_id]['max_depth'], random_state=const.random_seed)
    clf.fit(dataset.X_train, dataset.y_train)
    ...
    shap_values = shap.TreeExplainer(clf).shap_values(dataset.X_train)
    shap.summary_plot(shap_values, dataset.X_train, plot_type="bar", show=False)
    plt.savefig("results/plots/clev_shap_male.png", bbox_inches='tight', dpi=600)

    # female
    logger.info("Prediction female ---------")
    dataset = Dataset('cleveland', data[experiment_id]['features'], data[experiment_id]['target'], gender_sep='female')
    clf = RandomForestClassifier(max_depth=data[experiment_id]['max_depth'], random_state=const.random_seed)
    clf.fit(dataset.X_train, dataset.y_train)
    ...
    shap_values = shap.TreeExplainer(clf).shap_values(dataset.X_train)
    shap.summary_plot(shap_values, dataset.X_train, plot_type="bar", show=False)
    plt.savefig("results/plots/clev_shap_female.png", bbox_inches='tight', dpi=600)
